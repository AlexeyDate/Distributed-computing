...
int total_bodies; // Number of total points in file

int bodies; // Number of evaluated points per time step
int timeSteps;
...
FILE *fp_thread_1; // File for time results from thread 1
FILE *fp_thread_2; // File for time results from thread 2
		   
FILE *fp_out; // File for calculated values
double start_time;
double end_time;
...
void initiateSystem(char *fileName)
{
    ...
    fscanf(fp, "%lf%d%d", &GravConstant, &total_bodies, &timeSteps);
...
int main(int argc, char *argv[])
{
	...
	double total_time;
    	...
    	FILE *fp_thread_1 = fopen(argv[2], "w");
	FILE *fp_thread_2 = fopen(argv[3], "w");

	max_bodies = atoi(argv[4]);
	points_per_step = atoi(argv[5]);
	bodies = 0;
	pthread_t* thread_handles = NULL;
	
	for (int current_iter = 0; current_iter < max_bodies / points_per_step; current_iter++)
	{
		bodies += points_per_step;
		
		initiateSystem(argv[1]);
		thread_count = atoi(argv[2]);

		printf("********** Current evaluated iteration: %d **********\n", current_iter + 1);
		...
		printf("Total working time: %lf\n\n", total_time);
		fprintf(fp_thread_1, "%d %lf\n", bodies, total_time);
